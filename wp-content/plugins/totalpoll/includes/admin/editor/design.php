...
<div class="design section <?php echo !isset($last_opened_tabs['design']) ? 'collapsed' : '' ?>">
    <input type="checkbox" name="tp_opened_tabs[design]"
           class="tab-state" <?php checked(isset($last_opened_tabs['design']), true); ?>>
            ...
            <div class="settings-sections">
                <div class="settings-section toggled" data-toggle="section-current-settings">
                                    ...
                                    <optgroup label="<?php echo $template->name; ?>">
                                        <?php if (isset($presets->{$slug})): ?>
                                            <?php foreach ($presets->{$slug} as $name => $preset): $preset_id = $slug . '-preset-' . $name; ?>
                                                <option
                                                    value="<?php echo $preset_id; ?>" <?php selected($preset_id, $current_preset); ?>><?php echo $name; ?></option>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </optgroup>
                                <?php endforeach; ?>
                    ...
                    <p>
                        <button type="submit" class="button button-primary"
                                name="tp_options[template][preset][new]"><?php _e('Save as', TP_TD); ?></button>
                ...
                <?php if (isset($poll->template->settings['sections'])):
                    foreach ($poll->template->settings['sections'] as $section_id => $section): ?>
                        ...
                        <div class="hide settings-section need-refresh"
                             data-toggle="section-<?php echo $section_id; ?>-fields">
                            <?php
                            unset($section['label']);
